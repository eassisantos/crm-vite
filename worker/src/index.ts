  ...
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/bootstrap') {
        return jsonResponse(await handleBootstrap(env));
      }
      ...
      if (documentDeleteMatch && request.method === 'DELETE') {
        ...
        return jsonResponse(await handleDeleteDocument(env, decodeURIComponent(caseId), decodeURIComponent(documentId)));
      }
      ...
      if (documentDownloadMatch && request.method === 'GET') {
        ...
        return await handleDownloadDocument(env, decodeURIComponent(caseId), decodeURIComponent(documentId));
      }

      if (request.method === 'POST' && url.pathname.startsWith('/api/ai/')) {
        return jsonResponse(await handleAiRequest(env, url.pathname.replace('/api/ai', ''), await request.json()));
      }

      return jsonResponse({ message: 'Rota não encontrada.' }, { status: 404 });
    } catch (error) {
